...
let listData = () => {
  ...
  readFile().then((response) => {
    ...
    $.each(response, (key, value) => {
      ...
      const inPriceRange =
        rangeCondition[0] <= placePrice && rangeCondition[1] >= placePrice;

      if (!city.includes(value.Ciudad)) city.push(value.Ciudad);

      if (!type.includes(value.Tipo)) type.push(value.Tipo);
      ...
      if (
        ...
        if (
          cityCondition == value.Ciudad &&
          typeCondition == value.Tipo &&
          inPriceRange
        )
          result.push(value);
      } else if (
        ...
        if (typeCondition == value.Tipo && inPriceRange) result.push(value);
      } else if (
        cityCondition != "Elige una ciudad" &&
        typeCondition == "Elige un tipo"
      ) {
        if (cityCondition == value.Ciudad && inPriceRange) result.push(value);
...
let setType = (type, typeCondition) => {
  ...
  if (typeCondition != "Elige un tipo") $("#selectTipo").val(typeCondition);
};
...
let saveFunction = (id) => {
  $.ajax({
    ...
    success: function (response) {
      if (response.code == "success") alert("Guardado con exito");
      else alert("Error");
...
let setQuery = () => {
  ...
  if (typeof typeCondition !== "string" || typeCondition === "Elige un tipo") {
    typeCondition = "";
  }
